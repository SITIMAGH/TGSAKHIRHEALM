...
<style>
    .swal2-modal {
        ...
        border: 2px solid #ff0501 !important;
    }

    .swal2-title {
        border: 2px solid #ff0501;
        border-radius: 8px;
    ...
    <div class="col-12 mb-3">
        <div class="card product">
            <div class="card-body">
                ...
                <div class="row">
                    <div class="col-12">
                        <h6 class="text-truncate text-center" style="margin: -.7rem auto;"
            ...
            <div class="card-footer text-center bg-warning">
                ...
                <span class="btn btn-sm btn-secondary btn-rounded w-100 mt-2">Sold Out</span>
                <?php else: ?>
                <button
                    class="btn btn-sm btn-danger btn-rounded w-50 mt-2 <?php echo e($item->end_booking != null && $item->end_booking != '0000-00-00 00:00:00' ? 'd-none' : ''); ?> <?php echo e((strtotime(date('H:i:s')) >= strtotime(explode('-', $item->time_sale)[0] . ':00') && strtotime(date('H:i:s')) <= strtotime(explode('-', $item->time_sale)[1] . ':00')) ? 'd-none' : ''); ?>"
                    id="<?php echo e('btn-booking-' . $item->variant); ?>" onclick="onBookingStatistik('<?php echo e($item->variant); ?>')">Pre
                    Order</button>
                <button
                    class="btn btn-sm btn-success btn-rounded w-50 mt-2 <?php echo e((strtotime(date('H:i:s')) >= strtotime(explode('-', $item->time_sale)[0] . ':00') && strtotime(date('H:i:s')) <= strtotime(explode('-', $item->time_sale)[1] . ':00')) ? '' : 'd-none'); ?>"
                    id="<?php echo e('btn-buy-' . $item->variant); ?>" onclick="onBuy('<?php echo e($item->variant); ?>')">Buy</button>
                <span
                    class="btn btn-sm btn-rounded w-50 mt-2 <?php echo e($item->end_booking != null && $item->end_booking != '0000-00-00 00:00:00' ? '' : 'd-none'); ?> <?php echo e((strtotime(date('H:i:s')) >= strtotime(explode('-', $item->time_sale)[0] . ':00') && strtotime(date('H:i:s')) <= strtotime(explode('-', $item->time_sale)[1] . ':00')) ? 'd-none' : ''); ?>"
                    id="<?php echo e('btn-pesan-' . $item->variant); ?>" style="background-color: #d920a1;">Ordered</span>
                <?php endif; ?>
    ...
    function acc_booking(variant){
        ...
        .then(res => {
            ...
            if(res.data.status == 200){
                ...
                // })
                Swal.fire({
                        iconHtml: '<img class="icon-sw2" src="<?php echo e(base_url()); ?>assets/icon-success.png"/>',
                        title: `<div class="text-sw2">${res.data.msg}</div>`,
                        showConfirmButton: false,
                        timer: 1500,
                        customClass: {
                            icon: 'no-borer-sw2'
                ...
                // })
                Swal.fire({
                        iconHtml: '<img class="icon-sw2" src="<?php echo e(base_url()); ?>assets/icon-failed.png"/>',
                        title: `<div class="text-sw2">${res.data.msg}</div>`,
                        showConfirmButton: false,
                        timer: 1500,
                        customClass: {
